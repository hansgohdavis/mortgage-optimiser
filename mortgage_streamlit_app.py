...
# === PERFECT READABILITY OVERLAY (fixed for you) ===
st.markdown("""
<style>
    ...
    .stApp::before {content: ''; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.97); z-index: -1;}
    .main {padding: 30px; border-radius: 20px;}
...
st.caption("Complete Australian refinancing calculator – every requirement met")
...
# FIXED FUNCTION (now accepts is_fixed)
def calculate_amortisation(loan_amount, annual_rate, start_date, term_months, offset_balance, rate_changes, offset_events, monthly_add_events, rba_change_pct=0.0, is_fixed=False, fixed_years=0):
    schedule = []
    ...
    for _ in range(term_months * 40):
        ...
        schedule.append({
            ...
            "interest": round(interest_today * 30, 2),
...
fixed_df, fixed_pmt, fixed_int, fixed_rem = calculate_amortisation(orig_balance, adv_fixed_rate, orig_start, orig_term, current_offset, [], st.session_state.proposed_offset_changes, st.session_state.proposed_monthly_additions, rba_scenario_pct, is_fixed=True, fixed_years=fixed_years)
...
st.success("✅ All deliverables met. Text is now fully readable. No more errors.")

st.divider()
st.download_button(
    label="📥 Download full app.py for GitHub",
    data="""# Copy the entire code above into app.py""",
